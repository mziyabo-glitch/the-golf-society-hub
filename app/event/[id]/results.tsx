...
export default function EventResultsScreen() {
  ...
  const calculateWinner = (): { memberId: string; memberName: string } | null => {
    ...
    const validResults = selectedPlayers
      ...
      .filter(
        (item): item is { memberId: string; memberName: string; score: number; useStableford: boolean } =>
          item !== null
      );
  ...
  const handleSaveResults = async () => {
    ...
    try {
      ...
      const resultsObj: {
        [memberId: string]: { grossScore: number; stableford?: number; strokeplay?: number; netScore?: number };
      } = {};
      selectedPlayers.forEach((player) => {
        const playerResults = results[player.id];
  ...
  return (
    <Screen>
      ...
      {selectedPlayers.length === 0 ? (
        <EmptyState title="No players selected" message="Go to Players screen to select players first" />
      ) : (
        <>
          {selectedPlayers.map((player) => {
            const ch = getCourseHandicap(player, selectedMaleTeeSet, selectedFemaleTeeSet);
