...
export default function ManageEventScreen() {
  ...
  if (!canAccessManageEvent) {
    return (
      <Screen>
        ...
        <EmptyState
          title="Manage Event access required"
          message="You do not have permission to access event management controls."
        />
      </Screen>
    );
  }
  ...
  return (
    <Screen contentStyle={styles.screenContent}>
      ...
      <ManageEventSection
        title="Event Setup"
        description="Summaries, fees, and tee ratings. Use Edit (top) to change details."
        showDivider={false}
      >
      ...
      <AppCard style={styles.card}>
        ...
        {formatRsvpDeadlineDisplay(event.rsvpDeadlineAt ?? event.rsvp_deadline_at) ? (
          <>
            ...
            <AppText variant="small" color="muted" style={[styles.cardBodyText, { marginTop: 4 }]}>
              {getRsvpDeadlineDisplayTimeZone()
                ? `Displayed in ${getRsvpDeadlineDisplayTimeZone()}; invite open/closed compares server UTC to the stored instant.`
                : "Displayed in your local timezone; invite open/closed compares server time to the stored instant."}
            </AppText>
          </>
        ) : null}
      </AppCard>
      ...
      <ManageEventSection
        ...
        description="Roster, RSVPs, and playing list for this society."
      >
      {(canManageEventRoster || canEditEvent) && (
        <AppCard style={styles.card}>
          ...
          <AppText variant="small" color="muted" style={[styles.cardBodyText, { marginTop: spacing.xs }]}>
            “No reply” counts society members with no fee row on this event for this society (guests are in the totals above).
          </AppText>
      ...
      <ManageEventSection
        ...
        description="Record fees and confirm who is eligible for the tee sheet."
      >
        ...
        <AppCard style={styles.card}>
          ...
          <View style={styles.paidHeader}>
            <View style={styles.paidHeaderMain}>
              <AppText variant="subheading">Payment &amp; status</AppText>
              <AppText variant="small" color="secondary" style={styles.cardBodyText}>
                {detailIsJointEvent
                  ? "Only this society’s members and guests appear here. Switch society in the header for the other club. Marking paid confirms attendance."
                  : "Marking paid confirms attendance."}
              </AppText>
      ...
      {((canAssignPrizePoolSettings && event && eventId) || (showPrizePoolEventCard && eventId)) ? (
      <ManageEventSection
        title="Prize Pools"
        description="Enable the pot, assign a Pot Master, and open pool administration."
      >
      {canAssignPrizePoolSettings && event && eventId ? (
        <AppCard style={{ borderRadius: radius.md, marginTop: spacing.sm }}>
          ...
          <AppText variant="caption" color="secondary" style={{ marginBottom: spacing.sm }}>
            Members opt in from home; Pot Master confirms entrants. Does not change main event fees or attendance.
          </AppText>
          <View
            style={{
              flexDirection: "row",
      ...
      <ManageEventSection
        title="Results"
        description="Official scores and Order of Merit points (Captain / Handicapper)."
      >
        ...
        <AppCard style={[styles.actionCard, styles.managePrimaryActionCard]}>
          <View style={styles.actionRow}>
            ...
            <View style={styles.actionContent}>
              ...
              <AppText variant="caption" color="secondary">
                Official scores and OOM points
              </AppText>
            </View>
            <Feather name="chevron-right" size={20} color={colors.textTertiary} />
          </View>
      ...
      {canSendEventInvites ? (
        <AppCard style={styles.card}>
          ...
          <AppText variant="small" color="muted" style={[styles.cardBodyText, { marginBottom: spacing.sm }]}>
            Share a link — guests can respond without the app.
          </AppText>
      ...
      {canSharePaymentLists && societyId && event ? (
        <AppCard style={styles.card}>
          ...
          <PrimaryButton
            icon={<Feather name="share-2" size={16} color={colors.textInverse} />}
...
const styles = StyleSheet.create({
  ...
  /** Subtle emphasis for the main row action in a manage section (border only; same handlers). */
  managePrimaryActionCard: {
    borderWidth: 1,
