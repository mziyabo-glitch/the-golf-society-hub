-- Allow strict Free Play scorecard-ready when course_rating + par_total exist but WHS slope is unknown.
-- Official club scorecards often publish SSS only (stored as course_rating) without slope.
...
active_rated_tees AS (
  ...
  WHERE ct.is_active IS DISTINCT FROM false
    ...
    AND (
      ct.slope_rating IS NULL
      OR (ct.slope_rating IS NOT NULL AND ct.slope_rating::numeric > 0)
    )
),
hole_stats AS (
  SELECT
...
COMMENT ON FUNCTION public.free_play_search_scorecard_ready_courses(text, integer) IS
  'Free Play course search: strict same-tee scorecard data; course_rating + par required; slope optional when null.';
