...
-- Add society_oom_name to event_societies if not present (for custom OOM display names)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'event_societies' AND column_name = 'society_oom_name'
  ) THEN
    ALTER TABLE public.event_societies ADD COLUMN society_oom_name text;
  END IF;
END $$;
