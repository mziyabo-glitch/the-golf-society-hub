-- Payment actions must be scoped to ONE society: the active society (client passes p_society_id).
-- Previous versions (074/075) used LIMIT 1 across host ∪ event_societies, which could return
-- the wrong membership row when the user belongs to multiple societies (e.g. Member in ZGS,
-- Captain in M4) and incorrectly raised "Only Captain or Treasurer can mark payments".
...
COMMENT ON FUNCTION public.mark_event_paid(uuid, uuid, uuid, boolean, integer) IS
  'Captain/Treasurer in p_society_id marks payment for a member in that society only. Client passes active society.';
