-- Joint events: caller and target must be in host OR any participating society (event_societies).
-- Registration row uses the target member's society_id (not always the host).
-- Replaces 073 logic that only considered events.society_id and tried to map participants to host.
...
BEGIN
  ...
  -- Captain/Treasurer in the event host society OR any society linked in event_societies (joint).
  SELECT m.id, m.role
  ...
  -- Target member must belong to host or a participating society; registration uses that society_id.
