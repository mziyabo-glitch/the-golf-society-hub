-- Fix: mark_event_paid should work when UI passes a member_id from another society row
-- for the same real person (joint/merged player lists). Resolve to host-society member row.
...
BEGIN
  ...
  -- If incoming member_id is not in event society, map it to equivalent host-society row.
  IF NOT EXISTS (
    SELECT 1
      FROM public.members
    ...
    SELECT m.user_id,
      ...
      INTO v_src_user_id, v_src_person_id, v_src_email
      FROM public.members m
    ...
    SELECT m2.id
     ...
     WHERE m2.society_id = v_society_id
       AND (
         ...
         OR (v_src_person_id IS NOT NULL AND m2.person_id = v_src_person_id)
...
COMMENT ON FUNCTION public.mark_event_paid(uuid, uuid, boolean, integer) IS
  'Captain/Treasurer: set paid. Resolves cross-society member rows to event-society member when possible; paid=true forces status=in.';
