-- Fix Free Play RLS recursion / 403s by using a SECURITY DEFINER membership function.

CREATE OR REPLACE FUNCTION public.free_play_user_in_round(p_round_id uuid, p_user_id uuid DEFAULT auth.uid())
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE r.id = p_round_id
      AND (
        r.created_by_user_id = p_user_id
        OR EXISTS (
          SELECT 1
...
GRANT EXECUTE ON FUNCTION public.free_play_user_in_round(uuid, uuid) TO authenticated;
...
CREATE POLICY free_play_rounds_select ON public.free_play_rounds
  ...
  USING (public.free_play_user_in_round(id));
...
CREATE POLICY free_play_rounds_update ON public.free_play_rounds
  ...
  USING (public.free_play_user_in_round(id))
  WITH CHECK (public.free_play_user_in_round(id));
...
CREATE POLICY free_play_round_players_select ON public.free_play_round_players
  FOR SELECT TO authenticated
  USING (public.free_play_user_in_round(round_id));
...
CREATE POLICY free_play_round_players_insert ON public.free_play_round_players
  FOR INSERT TO authenticated
  WITH CHECK (
    public.free_play_user_in_round(round_id)
    OR user_id = auth.uid()
  );
...
CREATE POLICY free_play_round_players_update ON public.free_play_round_players
  FOR UPDATE TO authenticated
  USING (public.free_play_user_in_round(round_id))
  WITH CHECK (public.free_play_user_in_round(round_id));
