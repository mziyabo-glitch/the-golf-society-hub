-- 037_join_society_rpc_returns_member.sql
-- Update join_society to return the joined member row (not just society_id).
...
COMMENT ON FUNCTION public.join_society(text, text, text) IS
  'Join a society by join code and return the joined member row.';
